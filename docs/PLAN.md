...
- ✅ **Phase 0 Weeks 1–5 complete.**
  ...
  - **W5 FIDE ingestion: 755,081 players in production DB**, trigram search returns Magnus Carlsen with similarity 1.0
...
- 🔄 **Scope decisions locked 2026-05-12** (separate plan file in `~/.claude/plans/`):
  - USCF next after FIDE — added to W6
  - Games corpus → **dedicated AWS RDS** in eu-central-1 (Phase 1 W1)
  - Chess.com → lazy fetch + 7d cache (spec default)
  - ChessBase → never a data source (licensing)
- ⏭ **Next code work — Phase 0 Week 6:** `/scout` MVP + USCF ingest + own-game PGN import.
...
| Week | Deliverables                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | **Provision AWS RDS Postgres in eu-central-1** (db.t4g.medium starter, ~$50/mo) — the dedicated games-corpus database (decision locked 2026-05-12: Supabase Pro 8GB can't hold 400–600GB Lichess corpus; Team tier $599/mo overkill). Migrate `games` / `positions` / `moves` / `player_position_stats` / `player_opening_stats` schema to RDS. Extend `apps/workers/src/db.ts` with a second connection (`GAMES_DATABASE_URL`). **Lichess monthly DB dump ingestion** (Inngest worker) — filter rated games ≥ 1500 in last 24 months, parse with `pgn-extract` or `python-chess`, dedupe via `(source, source_game_id)`, write to partitioned `games` on RDS. Token-bucket rate limiter (60/min Lichess) for live API. |
...
| Storage scale (games corpus) exceeds Supabase tier | **LOCKED 2026-05-12:** dual-DB architecture. Supabase (eu-central-1) holds app data + federation tables forever. Dedicated **AWS RDS** in eu-central-1 (~$50/mo db.t4g.medium starter) holds the 400–600GB games corpus. Provisioned in Phase 1 W1 before Lichess dumps land. Partition `games` by `played_at` month from day one. |
