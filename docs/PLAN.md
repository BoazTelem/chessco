...
> **Update note (2026-05-17):** Three big landings this weekend:
>
> 1. **WS-1..WS-12 mega-commit merged + audited** (commits `c60f460` → `7067ce8`). Adds wallet shell, fairplay admin queue + decide/reverse endpoints, GDPR account delete/export, sparring marketplace tables + routes, share-token + PDF stub for reports, legal-draft pack, runbook pack, transactional email + analytics + Sentry shims, ledger contract library. 4 BLOCKERs caught in audit and fixed before push: ban-evasion via cascade FK (now RESTRICT), unimplemented 30-day PII purge (now PII-nullified at soft-delete time), unbounded export OOM (now row-capped with truncation flags), and missing unban endpoint (`/api/fairplay/[id]/reverse` added). Migration 0042 forward-port made idempotent + `ban_actions` now RLS-enabled.
> 2. **AI provider migrated Anthropic → DeepSeek** across the entire codebase (commit `84377ed`). `packages/ai` now uses DeepSeek chat-completions over OpenAI-compatible HTTP; `apps/web/lib/scout/llm-providers.ts` simplified to DeepSeek-only; `@anthropic-ai/sdk` dependency dropped from `packages/ai/package.json` and `apps/web/package.json`; `.env.example` files cleaned. Cost change: $0 setup fee, pay-per-token (~$0.001 per evidence-prose call).
> 3. **Maia practice-bot phasing committed** (commits `8ea2309` + `a41ea8a` + `52d6595`). Phase A (generic ladder) approved at $10–25/mo Cloud Run budget; service scaffolded at `services/maia-inference/` (FastAPI + lc0 + Maia weights from CSSLab). Phase B (per-opponent fine-tuning) confirmed as the product wedge. Credit-mode rule for bot games: drill always casual, sandbox/OTB credit-eligible only when bot rating ≥ user verified rating. Full plan in [MAIA-DEPLOYMENT.md](MAIA-DEPLOYMENT.md); credit rules in [PRACTICE-CREDIT-MODE.md](PRACTICE-CREDIT-MODE.md).
>
> **Audit note (2026-05-14):** This section was reconciled against the codebase. Phase 3 marketplace MVP is largely shipped under the `/practice` routing (not `/challenges` — see "Naming divergences" below). The AI/SEO discoverability bundle (Phase 1 W2) **shipped 2026-05-14** (commit `4dc5a88`) — robots.ts, sitemap.ts, JSON-LD, llms.txt, dynamic OG cards, plus friendly slug routing for `/p/[id]`. See "Untouched phases & quick wins" at the end of this doc for the remaining gap report.
...
- 🔄 **Phase 2 Scout-Ready Pipeline — landing in stages (2026-05-14)**, separate plan file at `~/.claude/plans/task-for-machine-2-gentle-wand.md`. Goal: turn the crawled corpus into a player-identification engine. Five phases:
  - ✅ **Phase 1 (scout-ready flag) shipped 2026-05-14.** Migration `0007_handles_scout_ready.sql` adds `handles.scout_ready_at timestamptz` + partial index. Evaluator at `apps/workers/src/identification/scout-ready.ts` (`isChesscomHandleReady` / `isLichessHandleReady` / `markIfScoutReady` / `safeMarkIfScoutReady` / SQL-aggregate backfill). One-shot backfill marked **1,503 chess.com + 1,732 Lichess** accounts (105s on 491k queue handles). Definition: chess.com handle is ready iff every queue row is `done` AND ≥1 archive_month exists; Lichess iff its single queue row is `done`. Permanent errors disqualify (we have a known gap). The evaluator UPSERTS into `handles` so it grows naturally for transitively-discovered accounts. **Run `tsx src/identification/scout-ready.ts` periodically to refresh** as the crawler completes new handles.
  - 🔄 **Phase 2 (persisted per-account repertoires) in progress 2026-05-14.** Migrations `0008_player_repertoire_cache.sql` (initial table) + `0009_repertoires_depth.sql` (composite PK on `(player_id, color, depth)` to support both shallow + deep tiers). Build worker at `apps/workers/src/repertoires/build.ts` with `--depth N --limit N --handle X --platform X --rebuild` flags. Reads directly from `games + moves + positions` (not `player_position_stats` / `player_opening_stats` — those tables exist but are EMPTY; revisit when populated). Tree shape mirrors `apps/web/lib/prepare/tree-builder.ts` so `/prepare` UI can consume it 1:1. **Two-tier strategy:** `depth=12` for all 3,235 ready handles (matcher input), `depth=30` for top-100 by `games_seen` (deep `/prepare` trees). **Backfill running** for depth-12 in background — first handle (danielnaroditsky, 9,410 games last 12mo) built ~30k nodes per color in 608s; smaller handles much faster.
  - 📋 **Phase 3 (`/prepare` bidirectional sync) queued.** New API `apps/web/app/api/prepare/games/route.ts` returns DB-cached games + tree; client merges with live API delta; new endpoint `apps/web/app/api/prepare/enqueue/route.ts` POSTs back to crawl queue. `/prepare` page-load goes from "fetch + parse + render" to "render-then-merge" — sub-second first paint when corpus is hot.
  - 📋 **Phase 4 (tighten existing PGN matcher) queued.** `apps/workers/src/stage3/match.ts` `rankBySampleGames()` filtered to `scout_ready_at IS NOT NULL` only; second pass uses `player_repertoires.tree` for position-by-position overlap (not just first-12-plies cosine).
  - 📋 **Phase 5 (LLM-primary matcher) queued.** Replace "algorithmic ranks → LLM verdicts top 3" with "algorithmic narrows to top 50 → LLM ranks all 50 from raw fingerprints" in `apps/web/lib/scout/evidence-prose.ts`. Uses DeepSeek's automatic server-side prompt cache keyed on the candidate block; per-PGN cache in `identification_queries`. Graceful fallback to algorithmic-only if LLM call fails or budget exceeded.
...
**Backend:** Supabase (Postgres + Auth + Storage + Realtime + pgvector + pg_trgm) • Drizzle ORM • Inngest workers • Upstash Redis • Stockfish on Cloud Run • **DeepSeek** (chat-completions; deepseek-chat default, deepseek-reasoner for narrative depth) • **Maia** (GPL-3.0; via lc0 in a Cloud Run inference service — see [MAIA-DEPLOYMENT.md](MAIA-DEPLOYMENT.md)) • Resend + Loops • Sentry + PostHog + BetterStack
...
| 8    | DeepSeek prompt library (`packages/ai/src/prompts/prep-summary.ts`, `evidence.ts`, `style-fingerprint.ts`, `risk-paragraphs.ts`, `help-chat.ts`) — coach not analyst, no inventing moves, structured JSON input. All prompts default to `deepseek-chat`; escalate prep-summary or risk-paragraphs to `deepseek-reasoner` via `runPrompt`'s `modelOverride` when narrative depth matters more than latency. Versioned prompts + eval harness (B11 in `apps/workers/src/eval/`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Phase 2 Scout-Ready Pipeline (extension, in progress 2026-05-14)

This is an architectural enhancement that landed alongside the original Phase 2 plan above. Rationale: with the chess.com + Lichess crawlers producing a ~2.4M-game / ~485k-distinct-handle corpus, we have enough data to turn the existing Stage 3 PGN matcher (cosine similarity over `style_features`) into a meaningfully better tool — but only for accounts whose corpus is "complete enough." The five phases below tie scout-ready signal → persisted repertoire trees → tightened matcher → LLM-primary ranking. Local plan file: `~/.claude/plans/task-for-machine-2-gentle-wand.md`.

| Phase | Status         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | ✅ shipped     | **Scout-ready flag.** Migration `0007_handles_scout_ready.sql` adds `handles.scout_ready_at` + partial idx on `(platform, scout_ready_at)`. Evaluator: `apps/workers/src/identification/scout-ready.ts`. A handle is ready iff (chess.com) every queue row is `done` AND ≥1 archive_month exists, or (Lichess) its single queue row is `done`. Permanent errors disqualify. SQL-aggregate backfill runs in ~105s on 491k queue handles. **Result 2026-05-14:** 1,503 chess.com + 1,732 Lichess marked ready. The evaluator UPSERTS into `handles`, so the canonical entity table grows naturally for transitively-discovered accounts via opponent discovery.                                                                                                                                            |
| 2     | 🔄 in progress | **Persisted per-account repertoires.** Migrations `0008_player_repertoire_cache.sql` + `0009_repertoires_depth.sql` give `player_repertoires (player_id, color, depth, tree jsonb, games_window int, built_at)`. Build worker `apps/workers/src/repertoires/build.ts` reads `games + moves + positions` (12-month window), reuses the `apps/web/lib/prepare/tree-builder.ts` algorithm inline. **Two tiers:** depth-12 for all ready handles (matcher input), depth-30 for top-100 by `games_seen` (deep `/prepare` trees). Tree shape mirrors the web client so `/prepare` can consume directly. Note: `player_position_stats` / `player_opening_stats` exist (migration 0001) but are empty — revisit when those are populated; storage shape lets the worker switch source without changing the JSON. |
| 3     | 📋 queued      | **`/prepare` bidirectional sync.** New `apps/web/app/api/prepare/games/route.ts` returns whatever the corpus has + cached tree from `player_repertoires`. The page renders immediately, then the existing client-side fetcher fills any missing months and POSTs new games to `apps/web/app/api/prepare/enqueue/route.ts` which enqueues the handle. Outcome: sub-second first paint when corpus is hot; corpus grows whenever a user opens a new `/prepare` page.                                                                                                                                                                                                                                                                                                                                       |
| 4     | 📋 queued      | **Tighten existing PGN→account matcher.** `rankBySampleGames()` in `apps/workers/src/stage3/match.ts` filtered to `scout_ready_at IS NOT NULL` candidates only; add a second comparison pass using the persisted repertoire tree for position-by-position overlap (not just first-12-plies cosine). Reweight `combined_score` to fold in the new signal. UI consumes the existing output unchanged.                                                                                                                                                                                                                                                                                                                                                                                                      |
| 5     | 📋 queued      | **LLM-primary matcher.** Today's flow: algorithmic ranks → LLM verdicts top 3. New: algorithmic narrows to top 50 → LLM ranks all 50 from compact fingerprints + repertoire summaries → returns top 3 with reasoning. Implemented in `apps/web/lib/scout/evidence-prose.ts`. Uses DeepSeek's automatic server-side prompt cache (the 50-candidate block is the cacheable prefix; only the input PGN's fingerprint changes per query). Per-PGN cache in `identification_queries`. Graceful fallback to algorithmic-only if the LLM call fails or budget exceeded.                                                                                                                                                                                                                                         |
| 6     | 📋 deferred    | **Auto-rebuild on refresh.** The 7-day Inngest refresh cron (`crawlRefreshChesscom` / `crawlRefreshLichess`) flips done queue rows back to pending; when the workers re-complete them, the `safeMarkIfScoutReady` hook updates `scout_ready_at`. After Phase 2 lands, also trigger a repertoire rebuild for the affected handle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

**Operational note (2026-05-14):** the `safeMarkIfScoutReady` runtime hook in the crawl `run.ts` files was authored but reverted (the in-flight code is hook-free); the backfill CLI is therefore the source of `scout_ready_at` updates today. If we want continuous updates without manually re-running the backfill, re-add the hook (5 call sites: 2 in `chesscom-crawl/run.ts`, 3 in `lichess-crawl/run.ts`).
...
## Phase 6 — Maia Practice Bots & Coach Features

**Goal:** The defensible product moment — practice against a human-like bot fine-tuned on your actual opponent's game history. Lichess and chess.com don't do this; ChessBase doesn't do this. This is the wedge in the Scout → Find → Practice → Pay → Improve loop. **Pulled forward 2026-05-17** — user green-lit Phase A this weekend; Phase B is the wedge and gets prioritized.

Phase 6 has TWO maia subphases and a credit-mode sublayer. Full specs:

- [MAIA-DEPLOYMENT.md](MAIA-DEPLOYMENT.md) — inference service deployment plan (Phase A + sketch of Phase B)
- [MAIA-INFERENCE.md](MAIA-INFERENCE.md) — HTTP contract between web app and inference service
- [PRACTICE-CREDIT-MODE.md](PRACTICE-CREDIT-MODE.md) — casual vs credit-stakes design rule

### Phase 6A — Generic Maia ladder (1–2 weeks of focus + dollars)

Lowest-risk path to a live bot. Maia 1500/1700/1900 buckets behind a single Cloud Run service. Drill + sandbox surfaces light up. No fine-tuning, no per-opponent weights yet.

| Week | Deliverables                                                                                                                                                                                                                                                                                           |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A1   | `services/maia-inference/` (scaffolded `a41ea8a`): FastAPI + lc0 v0.31.2 + Maia weights bundled at image build. Local docker build green; smoke `/healthz` + `/move` curl works.                                                                                                                       |
| A1   | Deploy to Cloud Run (us-central1, `--memory 2Gi --min-instances 1` — see MAIA-DEPLOYMENT.md Step 7). Seed `maia_weights` ladder rows in Supabase with fixed UUIDs matching the service env. Set `MAIA_INFERENCE_URL` in Vercel + local.                                                                |
| A2   | Wire `/practice/sandbox` to call the service: server route mints a game session, calls `getBotMove` per ply, persists PGN. `/practice/drill` calls the same service from prep-report positions. `/about/credits` page + `docs/legal/privacy.md` subprocessor entry for Maia + lc0 GPL-3.0 attribution. |
| A2   | Verify end-to-end against the ladder buckets. Phase 6A retro.                                                                                                                                                                                                                                          |

### Phase 6B — Per-opponent fine-tuning (THE WEDGE, 4–6 weeks)

The actual moat. Train a personalized Maia on a specific opponent's game history → bot that plays that human's style. Technique: McIlroy-Young et al. 2022 "Maia-Individual" (~50–55% move-matching vs that exact player). Need ~500+ games of the target to be useful; ~5000+ for high fidelity.

| Week | Deliverables                                                                                                                                                                                                                                                                                  |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1   | `apps/workers/src/maia/build-dataset.ts` — writes `(fen, played_uci, time_ms)` CSV from `games + moves` for a target profile/player. Storage at Supabase Storage bucket `maia-datasets` (service-role only). Hash + skip if unchanged.                                                        |
| B2   | `services/maia-train/Dockerfile` — lc0 train mode + CSSLab training scripts. Deploys as Cloud Run Job (NOT a service), with L4 or T4 GPU. Cost ~$2–8 per fine-tune.                                                                                                                           |
| B3   | `apps/workers/src/maia/dispatch-train.ts` — Inngest function: when `/prepare/otb` is requested for an opponent with ≥200 games, insert `maia_weights` row with `status='training'`, kick off the Cloud Run Job, on completion update the row with Supabase Storage URL of fine-tuned weights. |
| B4   | Inference service v2 — load weights on-demand by `weightsId`: keep an LRU of the most-recent ~50 sets in memory, cold-load from Supabase Storage on miss (~1–2s). Per-set weight file is ~30 MB.                                                                                              |
| B5   | `/prepare/otb` UX — "Training your opponent's bot — ETA 2 hours" status banner that flips to "Ready to practice" via Realtime when training_finished_at is set.                                                                                                                               |
| B6   | Phase 6B retro. Measure: training success rate, mean training duration, % of opponents with enough games.                                                                                                                                                                                     |

### Phase 6C — Credit-mode bot games (1 week, ships alongside or just after 6A)

Casual mode lands with Phase 6A. Credit mode layers on top per [PRACTICE-CREDIT-MODE.md](PRACTICE-CREDIT-MODE.md).

| Week | Deliverables                                                                                                                                                                                                                                                                                        |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1   | Migration: new table `practice_bot_games`, two new `credit_ledger_entries` categories `practice_bot_win`/`practice_bot_loss`, new `referenceType` value `practice_bot_game`. CHECK constraint enforcing `mode = 'casual' OR bot_rating >= user_rating` (load-bearing SQL gate against sandbagging). |
| C1   | `/practice/sandbox` start UI: casual + credit toggle. Credit toggle disabled (with tooltip) when chosen bucket < user's verified rating in the chosen time class. No linked chess.com / lichess account → credit mode unavailable + CTA to link.                                                    |
| C1   | Game-end settlement: single Postgres tx writes `practice_bot_games` result + `credit_ledger_entries` win/loss row. Win = +1 credit, loss = −1 credit, draw = 0 credit. Abandonment after move 5 = loss.                                                                                             |
| C1   | Daily cap on credit-mode games per user (start at 20/day; tune from usage). Server-authoritative clock + disconnect heartbeat to honor abandonment rule.                                                                                                                                            |

### Phase 6D — Spectator + Coach (3–5 weeks, after Maia is live)

| Week | Deliverables                                                                                                                                                                         |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1   | Spectator mode — read-only WebSocket subscribers with 10-move delay (anti-coaching); verified Silver+ trust tier gets no-delay.                                                      |
| D2–3 | **Coach accounts** — multi-student dashboards, assign prep reports to students, view student game history. `coach_students` table exists from WS-12; UI + dashboards still to build. |
| D4   | OTB tournament prep mode — FIDE ID integration, pre-tournament report bundle (build prep on all known pairings). Phase 6 retro.                                                      |

**Phase 6 Exit criteria:** Maia ladder playable in `/practice/sandbox` against 1500/1700/1900. Per-opponent fine-tune trains successfully for ≥80% of opponents with >500 games. Credit-mode games settle correctly with no off-by-one credit deltas. Coach accounts in use by ≥10 paying coaches.
...
- **Blog** (`content/blog/*.mdx`) — 2 articles/week for 6 months starting Phase 1 = 48 articles to build SEO base. DeepSeek-chat first drafts, human-edited.
...
| `packages/ai/src/prompts/`           | Versioned DeepSeek prompts — `prep-summary.ts`, `evidence.ts`, `style-fingerprint.ts`, `risk-paragraphs.ts`, `help-chat.ts` |
...
9. **AI redundancy** — DeepSeek-only today. Anthropic + OpenAI fallback paths considered but parked (would re-add provider seam to `apps/web/lib/scout/llm-providers.ts` and `packages/ai/src/client.ts`). Revisit if DeepSeek availability becomes a launch risk.
...
### Phase 2 — Identification depth (Scout-Ready Pipeline 20% shipped, rest 0%)

**Phase 2 Scout-Ready Pipeline (extension landed 2026-05-14)** — see the new sub-section under "Phase 2 — Identification Depth" above for full detail. Phase 1 of that pipeline (`scout_ready_at` flag + evaluator) is shipped; Phase 2 (persisted repertoires) is in active backfill; Phases 3–5 (`/prepare` bidirectional sync, tightened matcher, LLM-primary) are queued.

10. Engineered feature extractor at scale (200–500 features per player) — partly underway in `apps/workers/src/features/`; verify coverage. Note: the per-player aggregation tables `player_position_stats` / `player_opening_stats` exist (migration 0001) but are still **0 rows** — the Scout-Ready Pipeline reads from `games + moves + positions` directly until those are populated.
...
18. Chess.com continuous crawl queue (`apps/workers/src/chesscom/crawl/`) — **shipped under `apps/workers/src/chesscom-crawl/` (priority queue + workers + opponent discovery + 7-day refresh cron). Verify naming divergence with old plan text.**
