...
async function main() {
  ...
  try {
    const queue = await client<{ status: string; count: string }[]>`
      SELECT status, COUNT(*)::text AS count FROM lichess_crawl_queue
    ...
    >`
      ...
      FROM lichess_crawl_runs ORDER BY started_at DESC LIMIT 5
    `;
    ...
    console.log('--- lichess queue status ---');
    for (const r of queue) console.log(`  ${r.status.padEnd(18)} ${r.count}`);
    console.log('\n--- games by source ---');
    for (const r of games) console.log(`  ${r.source.padEnd(12)} ${r.n}`);
    console.log(`  distinct lichess handles seen: ${distinctHandles[0]!.n}`);
    console.log('\n--- last 5 lichess crawl runs ---');
    for (const r of runs) {
      console.log(
    ...
    console.log('\n--- 5 most recent lichess games ---');
    for (const r of sample) {
      console.log(
        `  ${r.played_at}  ${r.source_game_id.padEnd(12)}  ` +
