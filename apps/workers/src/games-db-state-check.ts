...
async function main() {
  ...
  try {
    const tables = await client<{ table_name: string }[]>`
      ...
      ORDER BY table_name
    `;
    console.log('0002 tables present:', tables.map((t) => t.table_name));
    ...
    const ext = await client<{ extname: string }[]>`
      ...
      ORDER BY extname
    `;
    console.log('extensions present  :', ext.map((e) => e.extname));

    const applied = await client<{ id: string }[]>`
      SELECT id FROM games_corpus_migrations ORDER BY id
    `;
    console.log('applied migrations  :', applied.map((m) => m.id));
  } finally {
